...
public final class SilkTouchAmethyst implements Listener {
	// Define valid tools
	Set<Material> validTools = EnumSet.of(Material.WOODEN_PICKAXE, Material.STONE_PICKAXE, Material.IRON_PICKAXE,
			Material.GOLDEN_PICKAXE, Material.DIAMOND_PICKAXE, Material.NETHERITE_PICKAXE);

	@EventHandler
	public void onBlockbreak(BlockBreakEvent event) {
		if ((event.getBlock().getType() == Material.BUDDING_AMETHYST)
				&& validTools.contains(event.getPlayer().getInventory().getItemInMainHand().getType())
